...
const CinematicLoading: React.FC<{ title: string; subtitle: string; label: string }> = ({ title, subtitle, label }) => {
  const words = title.split(' ');

  return (
    <div className="slayer-loading-scene min-h-screen bg-[var(--bg-base)] text-[var(--text-tertiary)] flex flex-col justify-center items-center font-mono select-none antialiased overflow-hidden relative">
      <div className="slayer-loading-grid" aria-hidden="true" />
      <div className="slayer-landing-orb slayer-landing-orb--one" aria-hidden="true" />
      <div className="slayer-landing-orb slayer-landing-orb--two" aria-hidden="true" />
      <div className="slayer-landing-orb slayer-landing-orb--three" aria-hidden="true" />

      <div className="relative z-10 w-[min(560px,calc(100vw-32px))] rounded-3xl border border-[var(--border)] bg-[var(--surface)]/80 p-8 text-center shadow-2xl shadow-black/60 backdrop-blur-xl slayer-breathing-panel">
        <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-[var(--border-strong)] bg-black/40">
          <Spinner size="lg" tone="primary" label={label} />
        </div>
        <div className="slayer-word-reveal justify-center text-sm font-black uppercase tracking-[0.22em] text-[var(--text-primary)]" aria-label={title}>
        ...
        <p className="mx-auto mt-4 max-w-md text-[11px] uppercase leading-relaxed tracking-[0.16em] text-[var(--text-tertiary)]">
          {subtitle}
        </p>
        <div className="mt-6 h-1 overflow-hidden rounded-full bg-white/[0.05]">
          <div className="slayer-loading-scan h-full w-1/2 rounded-full bg-[var(--accent-color)]/80" />
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  if (session === null) {
    return (
      <CinematicLoading
        ...
        subtitle="Verifying the secure session, restoring preferences, and preparing the terminal workspace."
      />
    );
  }
  ...
  if (!serverState) {
    return (
      <CinematicLoading
        label="Loading live market data"
        title="Loading live market data"
        subtitle="Syncing dealer positioning, ranked contracts, flow aggression, and quant surfaces before rendering."
      />
    );
  }
